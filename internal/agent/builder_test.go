...
var expectedDaemonConfig = WizeDaemonConfig{
	Apps: []App{
		{
			Name: "vdb1",
			Services: []Service{
...
func Test_WizeDaemonConfigBuilder(t *testing.T) {
	...
	cases := []struct {
		builder WizeDaemonConfigBuilder
		exp     WizeDaemonConfig
		message string
	}{
		{
			builder: *NewWizeDaemonConfigBuilder().SetAppBuilders([]*AppBuilder{appbuilder1, appbuilder3}),
...
func Test_daemonConfigBuild(t *testing.T) {
	...
	for _, c := range cases {
		...
		assert.Equal(t, len(c.exp.Apps), len(cfg.(*WizeDaemonConfig).Apps), c.message)
		assert.ElementsMatch(t, c.exp.Apps, cfg.(*WizeDaemonConfig).Apps, c.message)
	}
}
